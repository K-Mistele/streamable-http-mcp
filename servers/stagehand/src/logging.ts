...
const LOG_FILE = path.join(
	LOG_DIR,
	`stagehand-${new Date().toISOString().split("T")[0]}.log`,
);
...
export function logLineToString(logLine: LogLine): string {
	const timestamp = logLine.timestamp
		? new Date(logLine.timestamp).toISOString()
		: new Date().toISOString();
	const level =
		logLine.level !== undefined
			? logLine.level === 0
				? "DEBUG"
				: logLine.level === 1
					? "INFO"
...
export function log(
	...
	if (operationLogs.length > MAX_OPERATION_LOGS) {
		...
		const lastLogs = operationLogs.slice(
			operationLogs.length - (MAX_OPERATION_LOGS - 100),
		);
		operationLogs.length = 0;
		operationLogs.push(...firstLogs);
		operationLogs.push(
			`[...${operationLogs.length - MAX_OPERATION_LOGS} logs truncated...]`,
		);
